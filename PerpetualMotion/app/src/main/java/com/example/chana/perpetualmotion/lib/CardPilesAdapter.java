package com.example.chana.perpetualmotion.lib;
...
import com.example.chana.perpetualmotion.R;
...
public class CardPilesAdapter extends RecyclerView.Adapter<CardPilesAdapter.ViewHolder> {
    ...
    private void updateInnerCard(ViewHolder holder, int element) {
        if (mPILE_TOPS[element] != null) {
            populateAndShowInnerCard(holder, element);
        } else {
            clearAndHideInnerCard(holder);
        }
    }

    private void populateAndShowInnerCard(ViewHolder holder, int element) {

        Card currentCard = mPILE_TOPS[element];
        int currentColor = currentCard.getSuit().getColor();

        holder.tv_pile_card_rank_top.setTextColor(currentColor);
        holder.tv_pile_card_rank_top.setText(String.format(
        ...
        holder.cb_pile_card_checkbox.setChecked(mCHECKED_PILES[element]);
